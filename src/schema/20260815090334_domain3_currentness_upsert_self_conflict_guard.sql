-- Production ledger parity: 20260815090334_domain3_currentness_upsert_self_conflict_guard
-- Prevent a BEFORE INSERT currentness trigger from mutating the candidate_hash row
-- that INSERT ... ON CONFLICT is about to update.
...
comment on function atlas.enforce_live_data_signal_candidate_currentness_v1() is
  'Maintains one current row per semantic signal identity while excluding the same candidate_hash from prior-current retirement so INSERT ON CONFLICT may safely replay an already-current candidate.';
