-- Preserve Atlas's immutable candidate identity at the Lighthouse transport
-- boundary. Lighthouse projects and reviews this record; it does not derive a
...
comment on function public.bridge_live_data_signal_candidates_v1(uuid, integer) is
  'Projects Atlas Domain 3 candidates into Lighthouse with immutable Atlas candidate ID, hash, semantic key, evidence references, and governed transport receipts. It does not promote candidates or create findings.';
