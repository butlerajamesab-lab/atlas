-- Activate the governed Atlas -> Lighthouse Domain 3 transport on projects
-- where the source-controlled transport function was not applied because the
-- synchronous PostgreSQL HTTP extension was absent.
--
-- Additive and replay-safe. No candidate, signal, evidence, or bridge history
    ...
    begin
      select *
        ...
        from extensions.http((
          ...
          array[
            extensions.http_header('Authorization', 'Bearer ' || v_config.target_service_key),
...
comment on function public.bridge_live_data_signal_candidates_v1(uuid, integer) is
  'Atlas-owned synchronous Domain 3 transport using encrypted bridge config and explicit Lighthouse JSON receipts. The HTTP extension prerequisite is installed by the same activation migration.';
