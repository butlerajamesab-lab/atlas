-- Atlas candidates retain source-native pre-verification states. Lighthouse's
-- governed current-signal table has a narrower verification/entity vocabulary.
-- Translate those states at the transport boundary without promoting them.
-- `unverified` and exact-match-required both become `unresolved`, preserving
-- the epistemic state while satisfying the downstream constitutional contract.
...
begin
  ...
  loop
    ...
    begin
      ...
      if jsonb_typeof(v_body) <> 'object'
        ...
        raise exception 'Lighthouse registration returned a malformed receipt: %', left(v_response.content, 1000);
      end if;

      v_lighthouse_record_id := nullif(v_body->>'live_data_signal_id', '')::uuid;
      if v_lighthouse_record_id is null then
        raise exception 'Lighthouse registration receipt contains no live_data_signal_id: %', left(v_response.content, 1000);
      end if;
  ...
  return jsonb_build_object(
    ...
    'state_projection', 'atlas_candidate_to_lighthouse_governed_state_v1',
