...
export function normalizeWaAgComplaint(record) {
  ...
  return observationEnvelope({
    signalType: 'state_ag_consumer_complaint',
    timestamp: openedDate,
    jurisdiction: 'us_state_wa',
    region: state ? `us_state_${String(state).toLowerCase()}` : 'us_state_wa',
    channel: 'wa_ag',
    sourceSystem: 'washington_attorney_general_consumer_protection',
    sourceUrl: WA_AG_SOURCE_URL,
    geography: { state, city, zip },
    payload: {
      external_id: id,
      complaint_id: firstValue(record, ['id']),
      business_name: business,
      business_category: category,
      naics: firstValue(record, ['naics']),
      naics_name: firstValue(record, ['naicsname', 'naics_name']),
      status,
      opened_date: openedDate,
      opened_year: firstValue(record, ['openedyear', 'opened_year']),
      estimated_savings: firstValue(record, ['estimatedsavings', 'estimated_savings']),
      actual_savings: firstValue(record, ['actualsavings', 'actual_savings']),
      business_street_1: firstValue(record, ['businessstreetline1', 'business_street_line_1']),
      business_street_2: firstValue(record, ['businessstreetline2', 'business_street_line_2']),
      business_city: city,
      business_state: state,
      business_zip: zip,
      geocode: firstValue(record, ['geocode']),
