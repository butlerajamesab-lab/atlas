...
export function normalizeOregonComplaint(record) {
  ...
  const status = firstValue(record, ['status', 'complaint_status']);
  ...
  return observationEnvelope({
    ...
    timestamp: openedDate || closedDate,
    jurisdiction: 'us_state_or',
    region: state ? `us_state_${String(state).toLowerCase()}` : 'us_state_or',
    channel: 'oregon_doj',
    sourceSystem: 'oregon_department_of_justice_consumer_protection',
    sourceUrl: OREGON_COMPLAINTS_SOURCE_URL,
    geography: { state, city, zip },
    payload: {
      external_id: id,
      complaint_id: firstValue(record, ['complaint_number', 'complaint_id', 'id']),
      ...
      business_address: firstValue(record, ['business_address', 'address', 'respondent_address']),
      business_city: city,
...
export function ingestOregonConsumerComplaints({ limit = 5000, apiBaseUrl } = {}) {
  return ingestSocrataObservations({
    apiUrl: OREGON_COMPLAINTS_API_URL,
    sourceId: 'or_doj',
    jurisdictionId: 'us_state_or',
    moduleHint: 'consumer_protection',
    normalize: normalizeOregonComplaint,
    limit,
    apiBaseUrl,
  });
}
