 ...
 *
 * Canonical ownership:
 *   Scheduler → source adapter → replay-safe observation persistence
...
const ADAPTER_REGISTRY = [
  ...
  // Complaint / civic-pressure acquisition is intentionally multi-jurisdictional.
  // CFPB runs nationwide; state/local sources retain their own source identity.
...
async function runAdapter(adapter, { initiator = 'scheduler' } = {}) {
  ...
  try {
    ...
    state.lastResult = { ...result, action_receipt_hash: receipt.action_receipt_hash, receipt_error: receipt.receipt_error ?? null };
    state.errors = 0;
    console.log(`[scheduler] [OK] ${adapter.name} — event_delta=${eventDelta} source_count=${summary.source_count} outcome=${outcome} (${elapsed}ms)`);
    return state.lastResult;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    const completedAt = new Date().toISOString();
...
async function runDomain3Bridge() {
  ...
  try {
    ...
    domain3State.lastResult = {
      status: 'ok', elapsed: Date.now() - start,
...
function scheduleAdapter(adapter) {
  ...
  console.log(`[scheduler] Scheduled ${adapter.name} (${adapter.priority}) — first run in ${Math.round(initialDelay / 1000)}s, then every ${Math.round(adapter.intervalMs / HOUR)}h`);
}
...
export function startScheduler() {
  ...
  console.log(`[scheduler] Starting Atlas scheduler: state=${STATE} adapters=${ADAPTER_REGISTRY.length}`);
